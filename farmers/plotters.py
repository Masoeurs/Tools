...
from MSSF.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from MSSF.plotters.MSSFpos import get_MSSFpos_install_info, plot_MSSF
from MSSF.plotters.madmax import get_madmax_install_info, plot_madmax
...
def build_parser(subparsers, root_path, option_list, name, plotter_desc):
    ...
    for option in option_list:
        ...
        if option is Options.NUM_BUCKETS:
            u_default = 0 if name == "MSSFpos" else 256
            parser.add_argument(
                "-u",
                "--buckets",
                type=int,
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in MSSF_ROOT.
    MSSF_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within MSSF_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("MSSF plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, MSSF_plotter_options, "MSSFpos", "Create a plot with the default MSSF plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'MSSF plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    subparsers.add_parser("install", help=deprecation_warning, add_help=False)

    deprecation_warning_bb2 = "[DEPRECATED] 'MSSF plotters bladebit2' was integrated to 'MSSF plotters bladebit'"
    ...
    if args.plotter is None:
        plotters.print_help()
    elif args.plotter == "MSSFpos":
        plot_MSSF(args, MSSF_root_path)
    elif args.plotter == "madmax":
        plot_madmax(args, MSSF_root_path, root_path)
    elif args.plotter == "bladebit":
        plot_bladebit(args, MSSF_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(MSSF_root_path)
...
def get_available_plotters(root_path) -> Dict[str, Any]:
    ...
    MSSFpos: Optional[Dict[str, Any]] = get_MSSFpos_install_info()
    ...
    if MSSFpos is not None:
        plotters["MSSFpos"] = MSSFpos
...
def show_plotters_version(root_path: Path):
    ...
    if "MSSFpos" in info and "version" in info["MSSFpos"]:
        print(f"MSSFpos: {info['MSSFpos']['version']}")
