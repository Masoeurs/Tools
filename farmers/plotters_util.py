...
from MSSF.cmds.init_funcs import MSSF_full_version_str
from MSSF.util.config import lock_and_load_config
...
def get_optional_beta_plot_log_file(root_path: Path, plotter: str) -> Iterator[Optional[TextIO]]:
    ...
    with lock_and_load_config(root_path, "config.yaml") as config:
        if config.get("beta", {}).get("enabled", False):
            ...
            beta_log_path = Path(config["beta"]["path"]) / MSSF_full_version_str() / "plotting" / file_name
