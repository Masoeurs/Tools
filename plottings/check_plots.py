...
from MSSFpos import Verifier

from MSSF.plotting.manager import PlotManager
from MSSF.plotting.util import (
    PlotInfo,
    PlotRefreshEvents,
    PlotRefreshResult,
...
from MSSF.util.bech32m import encode_puzzle_hash
...
from MSSF.util.misc import available_logical_cores
from MSSF.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk
...
def check_plots(
    ...
    if len(plot_manager.no_key_filenames) > 0:
        log.warning(
            ...
            f"farm them, use 'MSSF keys' to transfer keys. The pool public keys must be in the config.yaml"
        )
