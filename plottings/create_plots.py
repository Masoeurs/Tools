...
from MSSF_rs import AugSchemeMPL, G1Element, PrivateKey
from MSSFpos import DiskPlotter

from MSSF.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from MSSF.plotting.util import Params, stream_plot_info_ph, stream_plot_info_pk
from MSSF.types.blockchain_format.proof_of_space import (
    calculate_plot_id_ph,
    calculate_plot_id_pk,
    generate_plot_public_key,
)
from MSSF.types.blockchain_format.sized_bytes import bytes32
from MSSF.util.bech32m import decode_puzzle_hash
from MSSF.util.keychain import Keychain
from MSSF.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
class PlotKeysResolver:
    ...
    async def get_farmer_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'MSSF keys add', 'MSSF keys generate' or provide a public key with -f"
            )
    ...
    async def get_pool_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'MSSF keys add', 'MSSF keys generate' or provide a public key with -p"
            )
