...
"""
run_block: Convert an encoded FullBlock from the MSSF blockchain into a list of transactions

As input, takes a file containing a [FullBlock](../MSSF/types/full_block.py) in json format
...
$ca_root is the directory containing your current MSSF config files
$hash is the header_hash of the [BlockRecord](../MSSF/consensus/block_record.py)
...
The information for each spend is an "NPC" (Name, Puzzle, Condition):
        ...
        "conditions": a list of condition expressions, as in [condition_opcodes.py](../MSSF/types/condition_opcodes.py)
...
from MSSF.util.config import load_config
from MSSF.util.default_root import DEFAULT_ROOT_PATH
